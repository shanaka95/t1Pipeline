"""
Top5 Clustering-Based Depression Severity Prediction Workflow with SMOTE
This module provides a complete pipeline for severity prediction using top5 clustering features.
...
class Top5SeverityWorkflowWithSMOTE:
    def __init__(self, processed_data_path='../processed_data/depression_processed_top5.csv',
        ...
        """Initialize the Top5 severity workflow with SMOTE comparison"""
    ...
    def load_data_and_features(self):
        """Load top5 clustering data and feature information"""
        print("Starting Top5 Depression Severity Prediction Workflow with SMOTE")
        print("="*80)
        ...
        print(f"Top5 data loaded: {self.df.shape}")
        print(f"Dataset Overview:")
        print(f"  - Samples: {self.df.shape[0]}")
        print(f"  - Features: {self.df.shape[1]}")
        print(f"  - Using Top5 clustering features")
        
        # Show feature categories
        print(f"\nTop5 Feature Categories:")
    ...
    def train_models_comparison(self, balance_methods=['none', 'smote']):
        ...
        print("\n" + "="*60)
        print("TRAINING TOP5 MODELS WITH SMOTE COMPARISON")
        print("="*60)
        
        # Initialize model trainers with top5 paths
        model_classes = {
        ...
        for balance_method in balance_methods:
            ...
            for model_name, model_class in model_classes.items():
                ...
                # Initialize trainer with top5 paths
                ...
                if model_name == 'XGBoost':
                    ...
                    model = trainer.train_random_forest_model(
                        X_train, y_train, X_test, y_test, 
                        model_name=f"top5_rf_{balance_method}",
                        tune_hyperparameters=False,
                        balance_method=balance_method,
                        use_class_weights=False
                    )
                    ...
                    model = trainer.train_svm_model(
                        X_train, y_train, X_test, y_test, 
                        model_name=f"top5_svm_{balance_method}",
                        tune_hyperparameters=False,
                        balance_method=balance_method,
                        use_class_weights=False
                    )
                ...
                print(f"Top5 {model_name} with {balance_method} completed!")
        
        print(f"\nAll Top5 models trained successfully!")
        print(f"Total model configurations: {len(self.all_models)}")
    
    def evaluate_all_models(self):
        """Comprehensive evaluation of all Top5 models with SMOTE comparison"""
        print("\n" + "="*60)
        print("COMPREHENSIVE TOP5 MODEL EVALUATION WITH SMOTE COMPARISON")
        print("="*60)
    ...
    def create_top5_visualizations(self):
        """Create visualizations specific to Top5 clustering results"""
        print("\nCreating Top5 SMOTE comparison visualizations...")
        
        # Create output directory
        os.makedirs('../severity_results/top5', exist_ok=True)
        
        # Set style
        plt.style.use('default')
        sns.set_palette("husl")
        
        # 1. Top5 SMOTE vs No-SMOTE Comparison
        self._plot_top5_smote_comparison()
        
        # 2. Top5 Model Performance by Balance Method
        self._plot_top5_balance_method_comparison()
        
        # 3. Top5 Confusion Matrices Comparison
        self._plot_top5_confusion_matrices_comparison()
        
        print("Top5 SMOTE comparison visualizations saved to '../severity_results/top5/' directory")
    
    def _plot_top5_smote_comparison(self):
        """Plot Top5 SMOTE vs no-SMOTE performance comparison"""
        comparison_data = []
        ...
        for i, metric in enumerate(metrics):
            ...
            axes[i].set_title(f'Top5 {metric} Comparison: SMOTE vs No-SMOTE')
        ...
        plt.suptitle('Top5 Clustering Severity Prediction: SMOTE Impact Analysis', fontsize=16)
        plt.tight_layout()
        plt.savefig('../severity_results/top5/top5_smote_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _plot_top5_balance_method_comparison(self):
        """Plot detailed Top5 balance method comparison"""
        summary_data = []
        ...
        axes[0].set_title('Top5 F1-Score Comparison by Balance Method')
        ...
        axes[1].set_title('Top5 Accuracy Comparison by Balance Method')
        axes[1].set_ylabel('Accuracy')
        axes[1].set_xlabel('Model Type')
        axes[1].legend(title='Balance Method')
        axes[1].tick_params(axis='x', rotation=45)
        
        plt.suptitle('Top5 Clustering Severity Prediction Performance Analysis', fontsize=14)
        plt.tight_layout()
        plt.savefig('../severity_results/top5/top5_balance_method_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _plot_top5_confusion_matrices_comparison(self):
        """Plot confusion matrices for Top5 SMOTE vs no-SMOTE"""
                ...
                for model_results in self.all_results.values():
                    for sub_model_name, results in model_results.items():
                        if config_name in f"{model_results}_{sub_model_name}":
                            ...
                            axes[i].set_title(f'Top5 Best {model_name} {title_suffix}\n(F1: {f1_score:.3f})')
                            axes[i].set_xlabel('Predicted Severity')
                            axes[i].set_ylabel('Actual Severity')
                            break
            
            plt.suptitle('Top5 Clustering Severity Prediction: Best Model Comparison', fontsize=14)
            plt.tight_layout()
            plt.savefig('../severity_results/top5/top5_confusion_matrices_smote_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
    
    def generate_top5_impact_report(self):
        """Generate detailed report on SMOTE impact for Top5 clustering"""
        print("\n" + "="*60)
        print("TOP5 CLUSTERING SMOTE IMPACT ANALYSIS REPORT")
        print("="*60)
        ...
        print(f"\nTop5 Best Performing Configurations:")
    ...
    def save_top5_results(self):
        """Save comprehensive Top5 results and comparison"""
        print("\nSaving comprehensive Top5 results...")
        
        os.makedirs('../saved_models/top5_severity', exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Save all models
        for config_name, models in self.all_models.items():
            for model_key, model in models.items():
                filename = f'../saved_models/top5_severity/top5_severity_{config_name}_{model_key}_{timestamp}.pkl'
                joblib.dump(model, filename)
                print(f"Top5 Severity {config_name} {model_key} saved to {filename}")
        
        # Save comparison results
        comparison_df = pd.DataFrame(self.comparison_results).T
        comparison_df.to_csv(f'../saved_models/top5_severity/top5_severity_smote_comparison_{timestamp}.csv')
        print(f"Top5 SMOTE comparison results saved to ../saved_models/top5_severity/top5_severity_smote_comparison_{timestamp}.csv")
        ...
        summary_file = f'../saved_models/top5_severity/top5_severity_smote_workflow_summary_{timestamp}.json'
        with open(summary_file, 'w') as f:
            json.dump(summary, f, indent=2, default=str)
        print(f"Top5 workflow summary saved to {summary_file}")
    
    def run_complete_workflow(self):
        """Run the complete Top5 comprehensive workflow with SMOTE comparison"""
        ...
        # Create Top5 SMOTE comparison visualizations
        self.create_top5_visualizations()
        
        # Generate impact report
        improvements = self.generate_top5_impact_report()
        
        # Save comprehensive results
        self.save_top5_results()
        
        # Final summary
        print("\n" + "="*80)
        print("TOP5 COMPREHENSIVE SEVERITY WORKFLOW WITH SMOTE COMPLETED SUCCESSFULLY!")
        print("="*80)
        
        print(f"\nTop5 Models Trained: {len(self.all_models)} configurations")
        print(f"Balance Methods: none, smote")
        
        best_model = max(self.comparison_results.items(), key=lambda x: x[1]['f1_score'])
        print(f"Best Overall Top5 Model: {best_model[1]['model_type']} with {best_model[1]['balance_method']} (F1: {best_model[1]['f1_score']:.4f})")
        
        print(f"\nOutput Files:")
        print(f"  - Models: ../saved_models/top5_severity/ directory")
        print(f"  - Visualizations: ../severity_results/top5/ directory")
        print(f"  - SMOTE comparison: top5_severity_smote_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        
        return self.all_models, self.comparison_results, improvements

def main():
    """Main function to run Top5 severity prediction workflow with SMOTE"""
    # Initialize workflow
