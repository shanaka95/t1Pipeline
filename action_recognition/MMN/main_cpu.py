...
def get_parser():
    ...
    parser.add_argument('--config', default='./config/train/MA52_J.yaml', help='path to the configuration f_Lineare')
...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.loss_type == 'CE':
            ...
            self.loss = LabelSmoothingCrossEntropy(smoothing=0.1).cpu()
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, index_t, label, index) in enumerate(process):
            ...
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cpu()  # 64,2,64,28,1 BS,C,T,V,1
                index_t = index_t.float().cpu()  # 64,64 BS,T
                label = label.long().cpu()  # 64, BS
        ...
        if save_model:
            ...
            self.print_log(
                f"\tSaved model checkpoint to {self.arg.model_saved_name + '-' + str(epoch + 1) + '-' + str(int(self.global_step)) + '.pt'}")

    def eval(self, epoch, save_score=False, loader_name=['train'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            self.print_log(
                '\tMean {} loss_fine of {} batches: {}.'.format(ln, len(self.data_loader[ln]), np.mean(loss_value)))
            ...
            epsilon = 1e-8
            each_acc = list_diag / (list_raw_sum + epsilon)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            if type(self.arg.device) is list:
                if len(self.arg.device) > 1:
                    weights = OrderedDict([['module.' + k, v.cpu()] for k, v in weights.items()])
            ...
            # Always save scores for test phase to get predictions
            self.eval(epoch=0, save_score=True, loader_name=['test'], wrong_file=wf, result_file=rf)
            self.print_log('Done.\n')
