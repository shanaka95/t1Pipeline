...
class ComprehensiveTop1DepressionPredictionWorkflow:
    def __init__(self, dataset_path='../datasets/ml_depression_dataset_top1.csv'):
    ...
    def load_data_and_features(self):
        ...
        self.action_features = [col for col in self.df.columns if col.startswith('action_class_')]
    ...
    def show_feature_details(self):
        ...
        print("\n" + "="*60)
        print("TOP1 ACTION CLASS FEATURE DETAILS")
        print("="*60)
        
        print(f"\nFeature Categories:")
        print(f"  - Action class features: {len(self.action_features)} features")
        ...
        action_means = self.df[self.action_features].mean().sort_values(ascending=False)
        for i, (feature, mean_val) in enumerate(action_means.head(10).items()):
            class_id = feature.split('_')[-1]
            print(f"  {feature} (Class {class_id}): {mean_val:.4f} mean percentage")
    ...
    def train_comparison_models(self, test_size=0.2, random_state=42):
        ...
        print("\n" + "="*80)
        print("TRAINING COMPARISON MODELS - DIFFERENT BALANCING APPROACHES")
        print("="*80)
    ...
    def train_all_models(self, test_size=0.2, random_state=42):
        ...
        print("\n" + "="*60)
        print("TRAINING ALL MODELS WITH OPTIMIZED CONFIGURATIONS")
        print("="*60)
        ...
        for model_name, model_info in self.all_models.items():
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                for i, row in feature_importance.head(10).iterrows():
                    ...
                    print(f"  {row['feature']} (Action Class {class_id}): {row['importance']:.4f}")
    ...
    def print_final_summary(self):
        ...
        print("\n" + "="*80)
        print("COMPREHENSIVE TOP1 ACTION CLASS WORKFLOW SUMMARY")
        print("="*80)
        
        print(f"\n🎯 APPROACH DETAILS:")
        print(f"  - Feature Type: Top1 Action Class (52 interpretable features)")
...
def main():
    """Main function to run the comprehensive top1 workflow"""
    # Initialize workflow
